...
async def admin_panel(message: Message):
    ...
    logger.info(f"Admin panel access attempt by user_id: {user_id}")
    ...
    with SessionLocal() as session:
        ...
        total_revenue = session.query(func.sum(Order.amount)).filter(Order.status == 'completed').scalar() or 0
...
async def admin_stats(callback: CallbackQuery):
    ...
    with SessionLocal() as session:
        ...
        # Финансы
        revenue_total = session.query(func.sum(Order.amount)).filter(Order.status == 'completed').scalar() or 0
        revenue_today = session.query(func.sum(Order.amount)).filter(
            Order.status == 'completed',
            Order.completed_at >= datetime.combine(today, datetime.min.time())
        ).scalar() or 0
        revenue_week = session.query(func.sum(Order.amount)).filter(
            Order.status == 'completed',
            Order.completed_at >= week_ago
        ).scalar() or 0
        revenue_month = session.query(func.sum(Order.amount)).filter(
            Order.status == 'completed',
        ...
        referral_earnings = session.query(func.sum(Referral.earned_amount)).scalar() or 0
